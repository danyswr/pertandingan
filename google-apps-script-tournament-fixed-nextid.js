    ...
    if (params.action === 'createAthleteGroup') {
      ...
      return ContentService
        .createTextOutput(JSON.stringify({
          ...
          id: groupId,  // Fixed: use groupId instead of nextId
...
function doGet(e) {
  try {
    ...
    if (params.action === 'getSubCategories' && params.mainCategoryId) {
      ...
      const data = sheet.getDataRange().getValues();
      const subCategories = data.slice(1)
    ...
    if (params.action === 'getAthleteGroups' && params.subCategoryId) {
      ...
      return ContentService
        .createTextOutput(JSON.stringify({success: true, data: groups}))
        .setMimeType(ContentService.MimeType.JSON);
    }
...
function initializeSheet() {
  ...
  console.log('Initializing tournament management sheets...');
  ...
  if (!groupAthleteSheet) {
    ...
    groupAthleteSheet.getRange(1, 1, 1, 9).setValues([['id_daftarKelompok', 'id_kelompokAtlet', 'nama_atlet', 'Berat_badan', 'Tinggi_badan', 'sabuk', 'umur', 'MB', 'Nomor']]);
    console.log('daftar_kelompok sheet created');
  }
  
  // Initialize atlets sheet if it doesn't exist
  let athletesSheet = spreadsheet.getSheetByName('atlets');
  if (!athletesSheet) {
    athletesSheet = spreadsheet.insertSheet('atlets');
    athletesSheet.getRange(1, 1, 1, 13).setValues([['id_atlet', 'nama_lengkap', 'gender', 'tgl_lahir', 'dojang', 'sabuk', 'berat_badan', 'tinggi_badan', 'kategori', 'kelas', 'isPresent', 'status', 'waktu_input']]);
    console.log('atlets sheet created');
  }
  
  console.log('Tournament management sheets initialized successfully');
}